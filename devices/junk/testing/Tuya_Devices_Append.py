...
def tuyaPlatform(apiRegion, apiKey, apiSecret, uri, token=None, new_sign_algorithm=True, body=None, headers=None):
    ...
    print(f"LINE 56 Token: {response}")
    try:
...
# First Message
print('LINE 79 TinyTuya Device Appender ' + ' [%s]' % (tinytuya.version))
print('')
...
if (config['apiKey'] != '' and config['apiSecret'] != '' and
    ...
    needconfigs = False
    # Second Message
    print("    " + "LINE 86 Existing settings:" + "\nAPI Key=%s \nSecret=%s\nDeviceID=%s\n Region=%s" %
    ...
    print(f"LINE 102 RESPONSE DICT ACCESS TOKEN : {response_dict}")
    ...
    print(f"\nLINE 123 Future Cloud Control json: {output}\n")
    ...
    print("\n\n" + "LINE 136 Device Listing\n")
    print(output)

    # Scan network for devices and provide polling data
    #print("\nLINE 140 Scanning local network for Tuya devices...")
    ...
    polling = []
    print("LINE 151 Polling local devices...")
    ...
    # print(f"LINE 205 Local Device json {output1}")
    ...
    print(f"LINE 229 LIGHTS: {lights}")
    switches = df[df['type'] == 'switch'].reset_index(drop=True)
    print(f"LINE 231 SWITCHES: {switches}")
    tuya = df[df['type'] == 'tuya'].reset_index(drop=True)
    print(f"LINE 233 SWITCHES: {tuya}")
