...
# Find out if we should poll all devices
    ...
    if (answer[0:1].lower() != 'n'):
        ...
        # output1 = json.dumps(current, indent=0)  # indent=4
        #print(f"LINE 269 Local Device json {output1}")
        # JSON Recieved Below
        # {
        # "timestamp": 1667852741.778813,
        # "devices": [
        output1 = {
            "timestamp": 1667867195.535403,
            "devices": [{
        ...
        #current = {'timestamp': time.time(), 'devices': polling}
        # output1 = json.dumps(current, indent=0)  # indent=4
        print(f"LINE 445 Local Device INSERTED json {output1}")
        # print(type(output1)) # Tuple
        # Add device sorter here #### MOVE IT OVER BELOW!!!!!!!!
        print('This is INSERTED Add DPS sorter here')
        #df = pd.read_json(output1)
        # Empty Frames if ['devices is removed]
        df = pd.json_normalize(output1['devices'])
        # df.to_dict(df)
        df = df.fillna(-1)
        ...
        print(f"LINE 480 LIGHTS: {lights}")
        print(f"LINE 481 SWITCHES: {switches}")
